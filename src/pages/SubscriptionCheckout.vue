<template>
  <div class="subscription-checkout-page content-section">
    ...
    <section class="section plans-section">
      ...
      <div class="membership-plans-grid" v-if="!loadingPrices && !pricesError">
        <div class="plan-card" v-for="plan in membershipPlans" :key="plan.priceId">
          ...
          <button class="btn btn-primary" @click="selectPlan(plan)" :disabled="selectedPlanId === plan.priceId">
    ...
    <section class="section payment-section" v-if="selectedPlanId">
      ...
      <p>You've selected: <strong>{{ selectedPlanName }}</strong> ({{ selectedPlanAmount }}/{{ selectedPlanInterval }})</p>
      ...
      <p v-if="paymentStatus" :class="{ 'success-message': paymentSuccess, 'error-message': !paymentSuccess }">
        {{ paymentStatus }}
      </p>
    </section>
  </div>
...
import { loadStripe, type Stripe, type StripeElements, type StripePaymentElement, type StripeLinkAuthenticationElement } from '@stripe/stripe-js';
...
const STRIPE_PUBLISHABLE_KEY = 'pk_test_51RcmipR2Cb1fdyD4axtO8nmGvavQwdg6pA9dpUqe2WEQFjvRdvdqru6e1z51AoCvVxTMpkK2vzBn5n44E5FVTQhG00gUu6DvyD'; // Replace
...
const createPaymentIntent = async (priceId: string): Promise<string> => {
  try {
    ...
    if (apiResponse.statusCode === 200) {
      ...
      throw new Error(errorData?.message || `Failed to create Payment Intent with status ${apiResponse.statusCode}.`);
    }
  } catch (e: unknown) {
...
const selectPlan = async (plan: MembershipPlan) => { // Made async
...
const handleSubscription = async () => {
  if (!selectedPlanId.value || !stripeInstance || !elementsInstance || !paymentElementInstance || !currentUser.value) {
    cardError.value = 'Please select a plan, and ensure payment form is loaded and you are logged in.';
    return;
  }
...
.status-message { color: #888; margin-top: 15px; }
